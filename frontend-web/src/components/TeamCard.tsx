...
const TeamCard = ({ member, index }: TeamCardProps) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={`bg-card rounded-2xl overflow-hidden shadow-sm border-2 transition-all duration-300 ${colorClasses[colorIndex]} ${member.isLead ? "md:col-span-2 lg:col-span-1" : ""}`}
    >
      {/* Image */}
      <div className="relative aspect-square bg-secondary overflow-hidden">
        ...
        {member.isLead && (
          <div className={`absolute top-3 right-3 ${accentColors[colorIndex]} text-primary-foreground px-3 py-1 rounded-full text-xs font-semibold`}>
            Core Team Lead
          </div>
        )}
      </div>
      ...
      {/* Info */}
      <div className="p-5">
        <h3 className="font-display text-lg font-semibold mb-1 line-clamp-1">{member.name}</h3>
        <p className="text-muted-foreground text-sm mb-1">{member.major}</p>
        <p className="text-muted-foreground text-xs mb-4">Batch {member.batch}</p>
